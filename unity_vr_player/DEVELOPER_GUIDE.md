...
这是一个基于 Unity 开发的跨平台 VR 视频播放器，支持 Windows、iOS、Android 三个平台。项目采用分层架构设计，实现了视频播放、VR 渲染、WebDAV 云存储集成、本地文件管理等核心功能。

### 核心特性

- **跨平台支持**: Windows、iOS、Android
- **VR 模式**: 360° 全景视频渲染、头部追踪、手势控制
...
```bash
# GitHub Actions 环境变量
UNITY_VERSION_CODE=${{ github.run_number }}
UNITY_VERSION_NAME=1.0.${{ github.run_number }}
```
...
```csharp
// 降低渲染分辨率
[SerializeField, Range(512, 4096)] private int renderTextureWidth = 1280;
[SerializeField, Range(512, 4096)] private int renderTextureHeight = 720;
```
...
- [DETAILED_PLAN.md](DETAILED_PLAN.md) - 详细实施步骤

### 版本历史

| 版本 | 日期 | 变更说明 |
|------|------|----------|
| 1.0 | 2026-03-23 | 初始版本，完整文档 |
